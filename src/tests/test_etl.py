...
class TestCSVLoader:
    ...
    def test_invalid_filename_pattern (self, tmp_path, caplog):
        ...
        bad_file.write_text("Client,Date,Channel,Campaign_id,Spend_usd\nDummy,2024-06-21,Google,camp_007,754.47")
        ...
        assert any("Invalid filename pattern. Check file." in msg for msg in caplog.messages)
        
    def test_valid_columns(self, tmp_path, caplog):
        bad_columns = tmp_path / "AD_SPEND_DUMMY_20250819.csv"
        bad_columns.write_text("Client,Date,,Campaign_id,Spend_usd\nDummy,2024-06-21,Google,camp_007,754.47")
        ...
        assert any(f"missing the required columns. Skipping file." in msg for msg in caplog.messages)
        assert any(loader.filename in msg for msg in caplog.messages)
        
class TestJSONLoader:

    def test_valid_filename(self, tmp_path):
        data = {"client":"Dummy", "date":"20250819", "channel":"Dummy", "impressions":99, "clicks":99, "conversions":99, "cost_per_click":1.00}
